...
class FileWatcherIntegrationTest {
    ...
    fun `multiple listeners receive events`() =
        runBlocking {
            ...
            assertTrue(events2.isNotEmpty(), "Second listener should receive events")
        }
    ...
    fun `unwatch stops events for directory`() =
        runBlocking {
            ...
            assertTrue(events.isEmpty(), "No events should be received after unwatching directory")
        }
    ...
    fun `adding and removing listeners works correctly`() =
        runBlocking {
            ...
            assertTrue(events1.isEmpty(), "Listener1 should not receive events after removal")
    ...
    fun `unwatch stops events for that directory only`(): Unit =
        runBlocking {
            ...
            assertTrue(events.none { it.path.startsWith(testDir) }, "No events should come from unwatched directory")
            assertTrue(events.any { it.path.startsWith(secondDir) }, "Events should come from watched directory")
